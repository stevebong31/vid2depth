...
class KittiRaw(object):
    ...
    def __init__(self,
               ...
               img_width=416,
               seq_length=3):
        static_frames_file = 'static_frames.txt'
        test_scene_file = 'test_scenes_' + split + '.txt'
...
class Endo(object):
    def __init__(self, dataset_dir, img_height=256, img_width=256, seq_length=3, sample_every=BIKE_SAMPLE_EVERY):
        self.dataset_dir = dataset_dir
        self.img_height = img_height
        self.img_width = img_width
        self.seq_length = seq_length
    ...
    def collect_frames(self):
        ...
        frames = []
        for video in video_list:
            im_files = glob.glob(os.path.join(self.dataset_dir, video, '*.png'))
            im_files = sorted(im_files, key=natural_keys)
            # Adding 3 crops of the video.
            frames.extend(['A' + video + '/' + os.path.basename(f) for f in im_files])
            frames.extend(['B' + video + '/' + os.path.basename(f) for f in im_files])
    ...
    def load_intrinsics(self, unused_frame_idx, cy):
        ...
        # # iPhone: These numbers are for images with resolution 720 x 1280.
        # Assuming FOV = 50.9 => fx = (1280 // 2) / math.tan(fov / 2) = 1344.8
        intrinsics = np.array([[2000, 0, 256 // 2],
                               [0, 2000, cy],
                               [0, 0, 1.0]])
        return intrinsics
